...
class FpgaController(wsgi.Controller):
    """The Hosts API controller for the OpenStack API."""
    def __init__(self):
        self.api = compute.HostAPI()
        super(FpgaController, self).__init__()
    ...
    def index(self, req):
        ...
        for service in services:
            if service.binary not in api_services:
                if service["host"].startswith("fpga"):
                    hosts.append({'fpga_name': service['host'],
...
class Fpgas(extensions.V21APIExtensionBase):
